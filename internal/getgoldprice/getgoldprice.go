...
type IGetGoPrice interface {
	GetLastPrice() (*Headline, error)
}

type GetGoldPrice struct{}
...
func (g *GetGoldPrice) GetLastPrice() (*Headline, error) {
	...
	h.BarSell = asFloat(sellTxt)
	h.BarBuy = asFloat(buyTxt)
	h.JewelrySell = asFloat(jwSellTxr)
	h.JewelryBuy = asFloat(jwBuyTxt)
	h.ReleaseAt = priceAtTxt
